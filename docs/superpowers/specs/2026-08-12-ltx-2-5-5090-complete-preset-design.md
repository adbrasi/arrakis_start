...
Add one active Arrakis Start preset that installs the complete official LTX 2.5 stack needed to test the development model, the two-stage development pipeline, and the official distilled pipeline on an RTX 5090.
...
3. `ltx25_distilled_official_5090.json`: official distilled 8-step T2V/I2V workflow adapted only to the optimized ComfyUI model filenames.
...
The diffusion VAE is the default decoder in every workflow. The convolutional VAE is installed as the official faster/lower-memory fallback, not placed in a parallel execution branch.
...
The preset requires current ComfyUI LTX 2.5 core support. Model loading uses the ComfyUI INT8 ConvRot checkpoints, with normal ComfyUI offload behavior for the RTX 5090's 32 GB VRAM.

The dev single-stage workflow runs full guided sampling before decoding with tiled `ltx-2.5-video-vae-bf16`. The dev two-stage workflow performs guided low-resolution generation, 2x latent spatial upscaling, distilled-LoRA refinement, then tiled diffusion-VAE decode. The distilled workflow retains the official 8-step sigma schedule.
...
- The optional convolutional VAE remains a manual loader selection; it does not silently replace the diffusion VAE after an out-of-memory failure.
